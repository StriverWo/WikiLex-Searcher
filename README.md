# WikiLex-Searcher
这是一款基于不同于传统文本匹配的具有文本相似度搜索特性的语义搜索引擎。基于 Sentence-Bert 大模型生成词嵌入（文本向量化），并以此通过 HNSW 算法（HNSWLib库）构建向量索引，以此来达到高维向量的快速相似度搜索。 
...
./lemserver
```
运行后会在终端输出（输出示意，还会生成部分日志信息以供调试这里不再显示）：
```Bash
...
Welcome to WikiLex-Searcher - A semantic search engine implemented in C++!
Starting WikiLex-Searcher server...
```

这样服务端就建立了，打开浏览器输入：127.0.0.1:8080 即可连接该搜索引擎（前端界面主要是由GPT生成）。  
* 登陆界面  
![image](https://github.com/user-attachments/assets/a2b94e46-0d9f-44b9-8075-8202a97263f4)

* 注册界面
![image](https://github.com/user-attachments/assets/fa4807a2-fa99-45b7-9a6f-05f734038e41)

* 登陆成功界面
![image](https://github.com/user-attachments/assets/11558c0b-a7b2-4c37-9992-abb8c55f7f87)
* 首页界面  
![image](https://github.com/user-attachments/assets/5e43ee94-a832-4212-b476-084e59820522)  
...
但似乎这种方式并不准确，因为一切信息都要以数据库中得为基准，而且这种用户信息像用户名和密码需要**实时性**，所以登录的时候始终需要从数据库中查找信息进行匹配，而不是从 Redis 缓存中。 **这个问题待后续更新。**

### 2. 登录时用户信息首先从布隆过滤器中获得：
原先设计登录时是从redis中匹配用户信息密码，然后根据匹配结果来确定登录是否成功。**但是，对于用户信息而言这是很重要的信息，不容有失，所以最好的办法是以 MySQL 中数据为准，所以在登录时需要从 MySQL 中匹配用户信息。**    
那么如果有不法分子拿着未知账号持续登录，那么这种登录请求都会打入到数据库上，那么很容易造成数据库崩溃。  
有什么好的办法呢？ **布隆过滤器！！！**

### 3. 布隆过滤器设计
参考博客： https://blog.csdn.net/qq_41125219/article/details/119982158 。

事实上在 Redis 中可以集成布隆过滤器，但在本项目中，自定义了实现了布隆过滤器的设计，源码放在src/bloomfilter.hpp中。
#### 谈谈设计
在 c++ 中位数组可以用 std::bitset 来定义，此外还需要一定数量的无偏函数用于生成哈希值，然后将位数组对应位置置"1"。
```cpp
std::bitset<1000000> bitset; // 位数组，大小可以调整
size_t hash_count; // 哈希函数的数量
```
* 关于std::bitset  
    这是一个模板类：它表示一个固定大小的 N 比特序列。bitset 可以用标准逻辑运算符操作，也可以转换成字符串和整数。在字符串表示法和移位操作的命名方向中，序列被认为是在右边有最低索引元素，就像整数的二进制表示法一样。
    它提供了方便的位操作功能，使得我们可以对二进制位进行高效的设置、清除、测试等操作。bitset大小在编译的时候确定，不可以更改。
    初始化操作如：
  ```cpp
      std::bitset<8> b(42); // 创建一个8位的bitset，并初始化为42（二进制表示为00101010）
      std::cout << b << std::endl; // 输出bitset的值，输出结果为 00101010
  ```
* 关于哈希函数
      本项目中自定义哈希函数，使用不同的种子生成多个哈希值：
  ```cpp
  ...
  /*
    hash_fn(element) ^ (i + 0x9e3779b9 + (hash_fn(element) >> 6))：对初始的哈希值进行一系列的位运算（异或、加法、右移），以此生成一个新的哈希值。这里结合了当前哈希函数的编号 i 和常量 0x9e3779b9，目的是确保每个哈希函数生成的哈希值不一样。
  */
    }
  ```
* 关于 std::hash 与 std::unordered_map
    std::hash是实现了仿函数的类模板，根据传入不同的数据类型，获得其哈希值。**对于C/C++内置数据类型，已经实现了其哈希函数，自定义数据类型需要定义其哈希值的求值方式。
    std::unordered_map  哈希表实现的关联容器，存储键值对。

**注意：**  
    * std::hash 是 C++ 标准库提供的一个模板类，它为许多标准类型提供了默认的哈希函数实现。不过，标准并没有规定这些哈希函数具体的实现细节，不同的编译器和标准库实现可能会采用不同的算法来生成哈希值，只要满足哈希函数的基本要求即可，这些要求包括：  
    对于相同的输入，哈希函数必须始终返回相同的哈希值。  
    哈希值应该尽可能均匀地分布在 size_t 类型的取值范围内，以减少哈希冲突的概率。  
    * 常见的实现思路
    虽然具体实现因编译器而异，但常见的字符串哈希算法思路如下：
    逐字符处理：遍历字符串中的每个字符，将字符的编码值与之前计算的部分哈希值进行某种运算。
    使用权重：为了让不同位置的字符对哈希值有不同的贡献，通常会给每个字符乘以一个权重。例如，使用一个质数作为权重，这样可以让哈希值更加分散。
    累积结果：将每个字符处理后的结果累积起来，最终得到整个字符串的哈希值。
